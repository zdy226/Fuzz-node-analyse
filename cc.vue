...
export default {
  ...
  methods: {
    ...
    // 新增代码  选择了节点分层 ，工具跟着节点分层变
    getNodelayer (value) {
      const params = {
        NodelayerName: value
      };
      this.getToolsList(params)
        .then((res) => {
          if (!res.datas.code === 0) {
            throw new Error(res.datas.message);
          }
          this.gongju_options = res.datas.tools;
          this.gongju_options.splice(0, 0, '所有工具');
          this.gongju_optionsAll = this.gongju_options[1];
          this.queryParams.tools = this.gongju_optionsAll[1];
        })
        .catch((error) => {
          this.$notice.error({
            title: '错误',
