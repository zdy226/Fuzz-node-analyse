      ...
      <div>
        <el-col :span="4">
          ...
          <el-select v-model="queryParams.emui">
            <el-option v-for="(item, index) in EMUI_options" :key="index" :value="item">{{ item }}</el-option>
          </el-select>
        </el-col>
        ...
        <el-col :span="4">
          ...
          <el-select multiple v-model="queryParams.version">
            <el-option v-for="(item, index) in banben_options" :key="index" :value="item">{{ item }}</el-option>
          </el-select>
        </el-col>
        ...
        <el-col :span="4">
          ...
          <el-select v-model="lingyu_optionsAll">
            <el-option v-for="(item, index) in lingyu_options" :key="index" :value="item">{{ item }}</el-option>
          </el-select>
        </el-col>
        ...
        <el-tab-pane label="领域测试">
          <div style="margin-top:30px;">
            <div style="margin-bottom:10px">
              <el-row>
                <el-col :span="20">
                  <span style="font-size:20px" class="bjh_h3">应用表</span>
                </el-col>
                <el-col :span="4">
                  <el-button size="small" @click="exportData">导出表格</el-button>
                  <el-button size="small" icon="el-icon-s-tools" @click="sheding">列表设定</el-button>
                </el-col>
              </el-row>
            </div>
            <el-table
              ...
              :cell-style="rowClass"
            >
              <el-table-column prop="target_name" label="包名" width="180">
                <template slot-scope="scope">
                  <router-link to="/safe" style="color:black">{{ scope.row.target_name}}</router-link>
                </template>
              </el-table-column>
              <el-table-column prop="domain" label="所属领域" width="180"></el-table-column>
              <el-table-column prop="app_version" label="应用版本"></el-table-column>
              <el-table-column
                ...
                label="应用类型"
              ></el-table-column>
              <el-table-column prop="tested" label="是否测试"></el-table-column>
              <el-table-column prop="success_components" label="测试完成组件/组件"></el-table-column>
              <el-table-column prop="problems" label="发现问题数"></el-table-column>
              <el-table-column prop="subnums" label="提单数"></el-table-column>
            </el-table>
            <!-- 分页 -->
            <div class="block" style="margin-top:20px;width: 100%;">
              <span
                class="demonstration"
              >显示第{{ reqParams.page }}-{{ reqParams.per_page }}条记录，共{{total}}条。每一页{{reqParams. per_page}}条</span>
              <el-pagination
                class="pager"
                background
          ...
          <div style="margin-top:30px;">
            <div style="margin-bottom:10px">
              <el-row>
                <el-col :span="20">
                  <div style="font-size:20px">应用表</div>
                </el-col>
                <el-col :span="4">
                  <el-button size="small" @click="exportData">导出表格</el-button>
                  <el-button size="small" icon="el-icon-s-tools" @click="sheding">列表设定</el-button>
            ...
            >
              ...
              <el-table-column prop="domain" label="所属领域" width="180"></el-table-column>
              <el-table-column prop="app_version" label="应用版本"></el-table-column>
              <el-table-column prop="app_type" label="应用类型"></el-table-column>
              <el-table-column prop="tested" label="是否测试"></el-table-column>
              <el-table-column prop="success_components" label="测试完成组件/组件"></el-table-column>
              <el-table-column prop="problems" label="发现问题数"></el-table-column>
              <el-table-column prop="subnums" label="提单数"></el-table-column>
          ...
          <div style="margin-top:30px;">
            ...
            >
              ...
              <el-table-column prop="target_name" label="包名" width="180">
                <template slot-scope="scope">
                  <!-- <router-link :to="' /ff/'+ target_name + app_version" style="color:black">{{ row.target_name}}</router-link> -->
                  <router-link to="/ff/" style="color:black">{{ scope.row.target_name}}</router-link>
                </template>
              </el-table-column>
              <el-table-column prop="domain" label="所属领域" width="180"></el-table-column>
              <el-table-column prop="app_version" label="应用版本"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    initPageTable() {
      ...
      getFuzzPonitTable(this.queryParams)
        .then((res) => {
          if (!res.datas.code === 0) {
            throw new Error(res.datas.message);
          }
    ...
    initBar() {
      ...
      let MyechartsTwo = this.$echarts.init(
        document.getElementById("MyechartsTwo")
      );
      //绘制图表
      window.addEventListener("resize", () => {
        MyechartsTwo.resize();
    ...
    initPie() {
      let PieChartsTwo = this.$echarts.init(
        document.getElementById("PieChartsTwo")
      );
      window.addEventListener("resize", () => {
        PieChartsTwo.resize();
      });
    ...
    ApplicationTableData() {
      // 获取表格列表数据
      getApplicationTableData(reqParams)
        .then((res) => {
          if (!res.datas.code === 0) {
            throw new Error(res.datas.message);
          }
    ...
    exportData() {
      let queryConditions = {};
      getFuzzTableData(queryConditions)
        .then((res) => {
          if (!res.datas.code === 0) {
